...
class GhosttyBot(commands.Bot):
    ...
    async def load_emojis(self) -> None:
        ...
        async with asyncio.TaskGroup() as group:
            for emoji in await self.fetch_application_emojis():
                group.create_task(self._update_app_emoji(emoji_files, emoji))
        ...
        async with asyncio.TaskGroup() as group:
            for missing_emoji in self._emojis:
                if self._emojis[missing_emoji] != "❓":
                    ...
                    continue
                group.create_task(
                    self._upload_missing_app_emoji(emoji_files, missing_emoji)
                )

        self.emojis_loaded.set()

    async def _update_app_emoji(
        self, emoji_files: dict[str, bytes], emoji: dc.Emoji
    ) -> None:
        if emoji.name in _OUTDATED_EMOJI_NAMES:
            logger.info("removing outdated emoji '{emoji}'", emoji=emoji.name)
            await emoji.delete()
            logger.info("removed outdated emoji '{emoji}'", emoji=emoji.name)
            return
        ...
        try:
            ...
            if not emoji_png.startswith(_PNG_HEADER):
                logger.error(
                    "BUG: app emoji '{emoji}' is not a png: it has probably been "
                    "modified by discord and hence a new update check method is "
                    "needed; assuming it is up-to-date in the interim",
                    emoji=emoji.name,
                )
    ...
    async def _upload_missing_app_emoji(
        self, emoji_files: dict[str, bytes], emoji: EmojiName
    ) -> None:
        logger.info("uploading missing emoji '{emoji}'", emoji=emoji)
        try:
            self._emojis[emoji] = await self.create_application_emoji(
                name=emoji, image=emoji_files[emoji]
            )
            logger.info("uploaded missing emoji '{emoji}'", emoji=emoji)
            logger.debug("loaded emoji '{emoji}'", emoji=emoji)
        except Exception as e:  # noqa: BLE001
            # Don't break the other missing emojis if uploading one fails.
            logger.opt(exception=e).error(
                "failed to upload missing emoji '{emoji}'", emoji=emoji
            )
