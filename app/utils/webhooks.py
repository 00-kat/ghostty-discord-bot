...
async def _format_reply(reply: dc.Message) -> dc.Embed:
...
def _format_missing_reference(message: dc.Message) -> dc.Embed:
    assert message.reference is not None
    if message.reference.type is dc.MessageReferenceType.forward:
        return dc.Embed(description="*Forwarded message was deleted.*").set_author(
            name="➜ Forwarded"
        )
