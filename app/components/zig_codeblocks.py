...
class ZigCodeblockActions(dc.ui.View):
    ...
    async def _reject_early(self, interaction: dc.Interaction, message: str) -> bool:
    ...
    async def dismiss(self, interaction: dc.Interaction, _: dc.ui.Button[Self]) -> None:
...
async def zig_codeblock_edit_hook(before: dc.Message, after: dc.Message) -> None:
    ...
    saved_msg: dc.Message | None = None
    for reply, new_content in zip_longest(replies, new_contents, fillvalue=None):
        ...
        if reply is None:
            ...
            if new_content is new_contents[-1]:
                ...
                msg = await after.channel.send(
                    new_content, allowed_mentions=dc.AllowedMentions.none()
                )
...
async def zig_codeblock_delete_hook(message: dc.Message) -> None:
