...
async def mention_entity(entity_id: int) -> str:
    msg, _ = await entity_message(
        ...
        cast("dsc.Message", SimpleNamespace(content=f"#{entity_id}"))
    )
    return msg
...
class Close(app_commands.Group):
    ...
    async def solved(
        self, interaction: dsc.Interaction, config_option: str | None = None
    ) -> None:
        if config_option:
            try:
    ...
    async def moved(self, interaction: dsc.Interaction, entity_id: int) -> None:
        await close_post(
            interaction,
            "moved",
    ...
    async def duplicate(self, interaction: dsc.Interaction, original: str) -> None:
    ...
    async def stale(self, interaction: dsc.Interaction) -> None:
        await close_post(interaction, "stale")

    @app_commands.command(name="wontfix", description="Mark post as stale.")
    async def wontfix(self, interaction: dsc.Interaction) -> None:
...
async def close_post(
    ...
    help_tags = {
        tag
        for tag in cast("dsc.ForumChannel", post.parent).available_tags
