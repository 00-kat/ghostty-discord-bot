...
async def section_autocomplete(_: dsc.Interaction, current: str) -> list[Choice[str]]:
    return [
...
async def page_autocomplete(
    interaction: dsc.Interaction, current: str
) -> list[Choice[str]]:
...
async def docs(
    interaction: dsc.Interaction, section: str, page: str, message: str = ""
) -> None:
    try:
        if not message or not isinstance(
            interaction.channel, dsc.TextChannel | dsc.ForumChannel
        ):
