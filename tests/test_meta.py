...
VS16 = b"\xef\xb8\x8f"
VS16_SUPPRESS_COMMENT = b"# test: allow-vs16"


def check_dir_for_vs16(path: Path) -> None:
    error_locations: list[tuple[Path, int]] = []
    for file in path.rglob("*.py"):
        for line_no, line in enumerate(file.read_bytes().splitlines(), 1):
...
def test_src_unwanted_vs16() -> None:
    check_dir_for_vs16(Path(__file__).parents[1] / "app")


def test_dummy_unwanted_vs16(tmp_path: Path) -> None:
    (tmp_path / "foo").mkdir()
    (tmp_path / "foo" / "bar.py").write_bytes(VS16)
    (tmp_path / "foo" / "baz.py").write_bytes(b"\xe2\x9d\xa4" + VS16)

    (tmp_path / "qux").mkdir()
    (tmp_path / "qux" / "foo.py").write_bytes(b"\xe2\x9d\xa4\xef")
    (tmp_path / "qux" / "bar.txt").write_bytes(VS16)
